...
我们将以上内容保存到本地文件（例如 `deployment.conf`）内，使用 SCP 即可上传至服务器的可用站点中：

```bash
$ scp ~/deployment.conf root@laravel-deployment.wi1dcard.cn:/etc/nginx/sites-available/
```
再创建软链接到启用站点中
```bash
ln -fs "/etc/nginx/sites-available/deployment.conf" "/etc/nginx/sites-enabled/deployment.conf"
```
...
- `/etc/nginx/sites-availabel/` 为存放 Nginx 站点配置的目录，该目录位于服务器。
- `/etc/nginx/sites-enabled/` 通过软链接的方式启用站点，例如：nginx自带的default站点我们一般都会删除，官方的建议是仅删除软链接而不删除源文件，这样nginx日后仍可以维护这一配置文件。
